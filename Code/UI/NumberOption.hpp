...
namespace Big::UserInterface
{
	...
	public:
			...
			m_Precision(precision)
		{
			...
			stringstream stream;
			stream << fixed << setprecision(1) << max;
			std::string pre = "~t~[~s~";

			std::string suf = " ~t~/ " + stream.str() + "]";

			std::strncpy(&m_Prefix[0], pre.c_str(), sizeof(m_Prefix) - 1);
			std::strncpy(&m_Suffix[0], suf.c_str(), sizeof(m_Suffix) - 1);
		}
