...
public:
	...
	inline const std::byte *GetRawResponseContent(void);
	...
	inline bool SetAdditionalDataToSend(std::byte *data, unsigned int dataSize);
				...
				while (!bGetReponseSucceed && iRetryTimes++ < INT_RETRYTIMES)
				{
					...
					if (bSendRequestSucceed)
					{
						...
						if (::WinHttpReceiveResponse(hRequest, NULL))
						{
							...
							m_pResponse = new std::byte[iMaxBufferSize];
							if (m_pResponse == NULL)
							{
								bRetVal = false;
								break;
							...
							do
							{
								...
								if (::WinHttpQueryDataAvailable(hRequest, &dwSize))
								{
									...
									std::byte *pResponse = new std::byte[dwSize + 1];
									if (pResponse != NULL)
									{
										memset(pResponse, 0, (dwSize + 1) * sizeof(std::byte));
										DWORD dwRead = 0;
											...
											&dwRead))
										{
											...
											{
												std::byte *pOldBuffer = m_pResponse;
												m_pResponse = new std::byte[iMaxBufferSize * 2];
												if (m_pResponse == NULL)
												{
...
bool WinHttpClient::SetAdditionalDataToSend(std::byte *data, unsigned int dataSize)
{
	...
	m_pDataToSend = NULL;
	m_pDataToSend = new std::byte[dataSize];
	if (m_pDataToSend != NULL)
	{
...
const std::byte *WinHttpClient::GetRawResponseContent(void)
{
	return m_pResponse;
}
