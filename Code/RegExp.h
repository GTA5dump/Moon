...
inline bool ParseRegExp(const wstring &regExp, bool caseSensitive, int groupCount, const wstring &source, vector<wstring> &result, bool allowDuplicate = false)
{
	...
	CAtlRegExp<> re;
	REParseError error = re.Parse((const ATL::CAtlRECharTraitsA::RECHARTYPE *)regExp.c_str(), caseSensitive);
	...
	while (bSucceed)
	{
		bSucceed = re.Match((const ATL::CAtlRECharTraitsA::RECHARTYPE *)pFrom, &mc, (const ATL::CAtlRECharTraitsA::RECHARTYPE **)&pTo);
		if (bSucceed)
		{
